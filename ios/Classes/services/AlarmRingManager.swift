...
class AlarmRingManager: NSObject {
    ...
    func start(
        registrar: FlutterPluginRegistrar, assetAudioPath: String, loopAudio: Bool,
        volumeSettings: VolumeSettings, onComplete: (() -> Void)?
    ) async {
        let start = Date()
        ...
        if volumeSettings.volumeEnforced {
            self.volumeEnforcementTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true)
            { _ in
                AlarmRingManager.shared.enforcementTimerTriggered(
                    targetSystemVolume: targetSystemVolume)
            }
        }

        guard
            let audioPlayer = self.loadAudioPlayer(
                registrar: registrar, assetAudioPath: assetAudioPath)
        else {
            await self.stop()
            return
        }
        ...
        if !volumeSettings.fadeSteps.isEmpty {
            ...
            self.fadeVolume(steps: [
                VolumeFadeStep(time: 0, volume: 0),
                VolumeFadeStep(time: fadeDuration, volume: volumeSettings.volume ?? 1.0),
            ])
        } else {
            audioPlayer.volume = Float(volumeSettings.volume ?? 1.0)
        }
    ...
    func stop() async {
        if self.volumeEnforcementTimer == nil && self.previousVolume == nil
    ...
    private func duckOtherAudios() {
        ...
        do {
            ...
            os_log(
                .error, log: AlarmRingManager.logger,
                "Error setting up audio session with option duckOthers: %@",
    ...
    private func mixOtherAudios() {
        ...
        do {
            ...
            try audioSession.setActive(true)
            os_log(
                .debug, log: AlarmRingManager.logger, "Play concurrently with other audio sources.")
        } catch {
            os_log(
                .error, log: AlarmRingManager.logger,
                "Error setting up audio session with option mixWithOthers: %@",
                error.localizedDescription)
        }
    }
    ...
    @objc private func enforcementTimerTriggered(targetSystemVolume: Float) {
        Task {
            ...
            if abs(currentSystemVolume - targetSystemVolume) > 0.01 {
                os_log(
                    .debug, log: AlarmRingManager.logger, "System volume changed. Restoring to %f.",
                    targetSystemVolume)
    ...
    private func loadAudioPlayer(registrar: FlutterPluginRegistrar, assetAudioPath: String)
        -> AVAudioPlayer?
    {
        let audioURL: URL
        if assetAudioPath.hasPrefix("assets/") || assetAudioPath.hasPrefix("asset/") {
            ...
            guard let audioPath = Bundle.main.path(forResource: filename, ofType: nil) else {
                os_log(
                    .error, log: AlarmRingManager.logger, "Audio file not found: %@", assetAudioPath
                )
                return nil
            }
        ...
        do {
            ...
            os_log(
                .debug, log: AlarmRingManager.logger, "Audio player loaded from: %@", assetAudioPath
            )
            return audioPlayer
        } catch {
            os_log(
                .error, log: AlarmRingManager.logger, "Error loading audio player: %@",
                error.localizedDescription)
            return nil
        }
    }
    ...
    private func fadeVolume(steps: [VolumeFadeStep]) {
        guard let audioPlayer = self.audioPlayer else {
            os_log(
                .error, log: AlarmRingManager.logger,
                "Cannot fade volume because audioPlayer is nil.")
            return
        }
        ...
        if !audioPlayer.isPlaying {
            os_log(
                .error, log: AlarmRingManager.logger,
                "Cannot fade volume because audioPlayer isn't playing.")
            return
        }
        ...
        for i in 0..<steps.count - 1 {
            ...
            Task {
                ...
                os_log(
                    .info, log: AlarmRingManager.logger, "Fading volume to %f over %f seconds.",
                    targetVolume, fadeDuration)
